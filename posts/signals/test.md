...
Find:
- Graph from eq (or viceversa)
- Even, odd part of $$x(t)$$
- Shifted, expanded time signal $$x(at + b)$$
- Whether $$x(t)$$ is finite energy or finite power signal
...
Find:
- Causal, stable, linear, time-invariant?
- $$h(t)$$ if not provided
- Given an expression for $$x(t)$$, the resultant $$y(t)$$
...
# Type 3: Fourier Series/Transform
...
04. 000000 IMPULSE Given impulse response in series/parallel, find non-zero range of y(t)
05. TYPE:3 F-SERIES Given x(t), find its FS, coefficients ak !IMPORTANT
06. TYPE:3 F-TRANSFORM Given x(t), find/sketch its FT, find FT when t = 0 !IMPORTANT
...
13. TYPE:6 DTFT given x[n], find its DTFT (see dtft properties)
14. TYPE:7 CONVOLUTION given x[n], h[n], find linear/circular convolution !IMPORTANT
